...
export default async function IkinciElUrunlerPage({
  ...
  return (
    <div className="bg-gray-50 flex-1">
      <div className="max-w-[1400px] w-[97%] mx-auto py-12">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-24">
              ...
              <ul className="space-y-2">
                ...
                {categories?.filter((c) => !c.parent_id).map((cat) => {
                  ...
                  return (
                    <li key={cat.id}>
                      <Link href={`/ikinci-el-urunler?c=${cat.id}`} className={`flex items-center justify-between text-sm py-2 px-3 rounded-lg transition-colors ${isCatActive ? 'bg-blue-50 text-blue-600 font-bold' : 'text-gray-600 hover:bg-gray-50 font-semibold'}`}>
                      ...
                      {subCats.length > 0 && (
                        <ul className="ml-4 mt-1 space-y-1 border-l-2 border-gray-100 pl-2">
                          {subCats.map((sub) => (
                            <li key={sub.id}>
                              <Link href={`/ikinci-el-urunler?c=${sub.id}`} className={`flex items-center justify-between text-xs py-1.5 px-3 rounded-md transition-colors ${cId === String(sub.id) ? 'text-blue-600 font-bold bg-blue-50/50' : 'text-gray-500 hover:text-blue-600 hover:bg-gray-50'}`}>
                                {sub.name}
                              </Link>
                            </li>
                          ))}
