  ...
  useEffect(() => {
    async function loadFooter() {
      try {
        ...
        if (json.data?.value) {
          setFooterSettings(json.data.value);
        } else {
          setFooterSettings({
  ...
  const saveFooter = async () => {
    ...
    try {
      const res = await fetch("/api/admin/settings", {
        ...
        body: JSON.stringify({ key: "footer_settings", value: footerSettings })
      });
      if (res.ok) {
        toast.success("Ayarlar başarıyla kaydedildi!", { icon: '⚙️' });
      } else {
        const r = await res.json();
        toast.error("Kayıt hatası: " + (r.error || "Bilinmeyen hata"));
      }
    } catch (err: any) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea rows={2} value={footerSettings.address} onChange={(e) => setFooterSettings({...footerSettings, address: e.target.value})} className={inputClass + " resize-none"} />
            </div>
            <div>
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-1"><Phone size={14}/> Telefon</label>
              <input type="text" value={footerSettings.phone} onChange={(e) => setFooterSettings({...footerSettings, phone: e.target.value})} className={inputClass} />
            </div>
            <div>
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-1"><Mail size={14}/> E-Posta</label>
              <input type="text" value={footerSettings.email} onChange={(e) => setFooterSettings({...footerSettings, email: e.target.value})} className={inputClass} />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="text" value={footerSettings.facebook} onChange={(e) => setFooterSettings({...footerSettings, facebook: e.target.value})} placeholder="https://facebook.com/sayfaniz" className={inputClass} />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">Twitter (X) URL</label>
              <input type="text" value={footerSettings.twitter} onChange={(e) => setFooterSettings({...footerSettings, twitter: e.target.value})} placeholder="https://twitter.com/sayfaniz" className={inputClass} />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">Instagram URL</label>
              <input type="text" value={footerSettings.instagram} onChange={(e) => setFooterSettings({...footerSettings, instagram: e.target.value})} placeholder="https://instagram.com/sayfaniz" className={inputClass} />
            </div>
          </div>
        </div>
        
        <div className="mt-6 flex justify-end">
          <button onClick={saveFooter} disabled={isSaving} className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-blue-700 transition disabled:opacity-70 text-sm shadow-sm">
